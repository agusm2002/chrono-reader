...
struct EPUBOPF: Codable {
    ...
    struct Metadata: Codable {
        let title: [EPUBString]?
        let creator: [EPUBString]?
        let metas: [EPUBMeta]
    }
    
    struct Manifest: Codable {
        let items: [EPUBManifestItem]
    }
    
    struct Spine: Codable {
        let itemrefs: [EPUBSpineItemRef]
        let toc: String?
        let direction: String?
    }
}

/// Elemento de metadata
struct EPUBMeta: Codable {
    let name: String?
    let content: String?
}

/// Elemento de manifest
...
/// Referencia en spine
struct EPUBSpineItemRef: Codable {
    let idref: String?
    let linear: String?
    let properties: String?
}
...
/// String con valor
struct EPUBString: Codable {
    let value: String
}

/// NCX
struct EPUBNCX: Codable {
    let navMap: EPUBNavMap?
}

/// NavMap
struct EPUBNavMap: Codable {
    let navPoints: [EPUBNavPoint]?
}

/// NavPoint
struct EPUBNavPoint: Codable {
    let navLabel: EPUBNavLabel?
    let content: EPUBContent?
    let navPoints: [EPUBNavPoint]?
}

/// NavLabel
struct EPUBNavLabel: Codable {
    let text: String?
}
